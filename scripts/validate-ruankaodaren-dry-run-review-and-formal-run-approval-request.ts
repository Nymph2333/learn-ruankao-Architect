/**
 * Phase 6.19 Dry-Run Review and Formal Run Approval Request validator.
 *
 * Validates the generated phase6_19_dry_run_review_and_formal_run_approval_request.json artifact
 ...
 * - formal_run_approval_status is not pending_human_review
 * - requested_decision is not approve_formal_offline_batch_run
 * - formal_run_approved is not false
 * - formal_run_executed is not false
 * - batch_run_executed is not false
 * - formal_execution_status is not approval_requested
 ...
 * - human_decision_fields.human_decision_recorded is not false
 * - human_decision_fields.decision_options does not have exactly 2 entries
 * - formal_run_approval_request.human_review_required is not true
 * - dry_run_review.review_status is not completed
 * - dry_run_review.review_outcome is not dry_run_passed_formal_run_eligible
 * - item_1_3.formal_run_eligible is not true
 * - item_9_1.formal_run_eligible is not false
 * - item_13_3.formal_run_eligible is not false
 * - planned outputs do not have status = planned
 * - prior_phase_reference.prior_phase is not 6.18
 ...
 *
 * Usage:
 *   pnpm validate:dry-run-review-and-formal-run-approval-request
 */
...
const schemaPath = resolve(repoRoot, "schemas/ruankaodaren-dry-run-review-and-formal-run-approval-request.schema.json");
const manifestPath = resolve(repoRoot, "data/manifests/phase6_19_dry_run_review_and_formal_run_approval_request.json");

function fail(message: string): never {
  console.error(`[validate:dry-run-review-and-formal-run-approval-request] ERROR: ${message}`);
  process.exit(1);
}
...
function main(): void {
  console.log("[validate:dry-run-review-and-formal-run-approval-request] Phase 6.19 Dry-Run Review and Formal Run Approval Request validator");
  ...
  if (schemaErrors.length > 0) {
    console.error("[validate:dry-run-review-and-formal-run-approval-request] Schema validation errors:");
    for (const err of schemaErrors) console.error(`  - ${err}`);
    fail("Schema validation failed");
  }
  console.log("[validate:dry-run-review-and-formal-run-approval-request] Schema validation: PASS");
  ...
  const dryRunReview = manifest["dry_run_review"] as Record<string, unknown> | undefined;
  if (!dryRunReview) fail("dry_run_review missing");

  const formalRunApprovalRequest = manifest["formal_run_approval_request"] as Record<string, unknown> | undefined;
  if (!formalRunApprovalRequest) fail("formal_run_approval_request missing");
  ...
  const checks: Array<[string, boolean]> = [
    // Top-level identity
    ["manifest_version = phase6.19", manifest["manifest_version"] === "phase6.19"],
    ["manifest_type = dry_run_review_and_formal_run_approval_request", manifest["manifest_type"] === "dry_run_review_and_formal_run_approval_request"],
    ["status = dry_run_review_and_formal_run_approval_request_created", manifest["status"] === "dry_run_review_and_formal_run_approval_request_created"],
    ["created_for_phase = 6.19", manifest["created_for_phase"] === "6.19"],
    ...
    // Formal run approval fields
    ["formal_run_approval_status = pending_human_review", manifest["formal_run_approval_status"] === "pending_human_review"],
    ["requested_decision = approve_formal_offline_batch_run", manifest["requested_decision"] === "approve_formal_offline_batch_run"],
    ["formal_run_approved = false", manifest["formal_run_approved"] === false],
    ["formal_run_executed = false", manifest["formal_run_executed"] === false],
    ["batch_run_executed = false", manifest["batch_run_executed"] === false],
    ["formal_execution_status = approval_requested", manifest["formal_execution_status"] === "approval_requested"],
    ...
    ["prior_phase_reference.prior_phase = 6.18", priorPhaseRef!["prior_phase"] === "6.18"],
    ["prior_phase_reference.prior_dry_run_status = completed", priorPhaseRef!["prior_dry_run_status"] === "completed"],
    ["prior_phase_reference.prior_dry_run_result = pass", priorPhaseRef!["prior_dry_run_result"] === "pass"],
    ["prior_phase_reference.prior_dry_run_executed = true", priorPhaseRef!["prior_dry_run_executed"] === true],
    ["prior_phase_reference.prior_formal_execution_status = not_started", priorPhaseRef!["prior_formal_execution_status"] === "not_started"],

    // Dry-run review
    ["dry_run_review.review_status = completed", dryRunReview!["review_status"] === "completed"],
    ["dry_run_review.review_outcome = dry_run_passed_formal_run_eligible", dryRunReview!["review_outcome"] === "dry_run_passed_formal_run_eligible"],
    ["dry_run_review.dry_run_result = pass", dryRunReview!["dry_run_result"] === "pass"],
    ["dry_run_review.dry_run_executed = true", dryRunReview!["dry_run_executed"] === true],

    // Item review results
    ["dry_run_review.item_1_3.status = execution_approved", ((dryRunReview!["item_review_results"] as Record<string, Record<string, unknown>>)["item_1_3"]?.["status"] === "execution_approved")],
    ["dry_run_review.item_1_3.dry_run_result = dry_run_scope_valid", ((dryRunReview!["item_review_results"] as Record<string, Record<string, unknown>>)["item_1_3"]?.["dry_run_result"] === "dry_run_scope_valid")],
    ["dry_run_review.item_1_3.formal_run_eligible = true", ((dryRunReview!["item_review_results"] as Record<string, Record<string, unknown>>)["item_1_3"]?.["formal_run_eligible"] === true)],
    ["dry_run_review.item_9_1.status = deferred_candidate", ((dryRunReview!["item_review_results"] as Record<string, Record<string, unknown>>)["item_9_1"]?.["status"] === "deferred_candidate")],
    ["dry_run_review.item_9_1.dry_run_result = deferred_not_assessed", ((dryRunReview!["item_review_results"] as Record<string, Record<string, unknown>>)["item_9_1"]?.["dry_run_result"] === "deferred_not_assessed")],
    ["dry_run_review.item_9_1.formal_run_eligible = false", ((dryRunReview!["item_review_results"] as Record<string, Record<string, unknown>>)["item_9_1"]?.["formal_run_eligible"] === false)],
    ["dry_run_review.item_13_3.status = quarantined_ineligible", ((dryRunReview!["item_review_results"] as Record<string, Record<string, unknown>>)["item_13_3"]?.["status"] === "quarantined_ineligible")],
    ["dry_run_review.item_13_3.dry_run_result = quarantined_ineligible", ((dryRunReview!["item_review_results"] as Record<string, Record<string, unknown>>)["item_13_3"]?.["dry_run_result"] === "quarantined_ineligible")],
    ["dry_run_review.item_13_3.formal_run_eligible = false", ((dryRunReview!["item_review_results"] as Record<string, Record<string, unknown>>)["item_13_3"]?.["formal_run_eligible"] === false)],

    // Formal run approval request
    ["formal_run_approval_request.requested_decision = approve_formal_offline_batch_run", formalRunApprovalRequest!["requested_decision"] === "approve_formal_offline_batch_run"],
    ["formal_run_approval_request.formal_run_approval_status = pending_human_review", formalRunApprovalRequest!["formal_run_approval_status"] === "pending_human_review"],
    ["formal_run_approval_request.formal_run_approved = false", formalRunApprovalRequest!["formal_run_approved"] === false],
    ["formal_run_approval_request.formal_run_executed = false", formalRunApprovalRequest!["formal_run_executed"] === false],
    ["formal_run_approval_request.human_review_required = true", formalRunApprovalRequest!["human_review_required"] === true],
    ["formal_run_approval_request.requested_formal_run_mode = offline_existing_source_packet_only", formalRunApprovalRequest!["requested_formal_run_mode"] === "offline_existing_source_packet_only"],

    // Formal run scope
    ["formal_run_scope.batch_id = phase6_1_batch_001", ((formalRunApprovalRequest!["requested_formal_run_scope"] as Record<string, unknown>)?.["batch_id"] === "phase6_1_batch_001")],
    ["formal_run_scope.no_additional_item_added = true", ((formalRunApprovalRequest!["requested_formal_run_scope"] as Record<string, unknown>)?.["no_additional_item_added"] === true)],
    ["formal_run_scope.existing_source_packet_boundary_sufficient = true", ((formalRunApprovalRequest!["requested_formal_run_scope"] as Record<string, unknown>)?.["existing_source_packet_boundary_sufficient"] === true)],

    // Human decision fields
    ["human_decision_fields.requested_decision = approve_formal_offline_batch_run", humanDecisionFields!["requested_decision"] === "approve_formal_offline_batch_run"],
    ["human_decision_fields.formal_run_approval_status = pending_human_review", humanDecisionFields!["formal_run_approval_status"] === "pending_human_review"],
    ["human_decision_fields.formal_run_approved = false", humanDecisionFields!["formal_run_approved"] === false],
  ...
  const formalRunScope = formalRunApprovalRequest!["requested_formal_run_scope"] as Record<string, unknown> | undefined;
  if (formalRunScope) {
  ...
  // Decision options must have exactly 2 entries
  const decisionOptions = humanDecisionFields!["decision_options"] as string[] | undefined;
  if (Array.isArray(decisionOptions)) {
    checks.push(["human_decision_fields.decision_options has 2 entries", decisionOptions.length === 2]);
    checks.push(["decision_options includes approve_formal_offline_batch_run", decisionOptions.includes("approve_formal_offline_batch_run")]);
    checks.push(["decision_options includes reject_formal_offline_batch_run", decisionOptions.includes("reject_formal_offline_batch_run")]);
  }

  // Human decision constraints must have 5 entries
  const humanConstraints = humanDecisionFields!["constraints"] as string[] | undefined;
  if (Array.isArray(humanConstraints)) {
    checks.push(["human_decision_fields.constraints has 5 entries", humanConstraints.length === 5]);
  }

  // Formal run output boundary — all 4 must be planned
  const outputBoundary = formalRunApprovalRequest!["requested_formal_run_output_boundary"] as Record<string, unknown> | undefined;
  if (outputBoundary) {
  ...
  // Rollback condition
  checks.push(["rollback_condition.phase6_19_rollback_needed = false", rollbackCondition!["phase6_19_rollback_needed"] === false]);
  ...
  checks.push(["audit_condition.current_formal_run_approval_status = pending_human_review", auditCondition!["current_formal_run_approval_status"] === "pending_human_review"]);
  checks.push(["audit_condition.current_formal_run_approved = false", auditCondition!["current_formal_run_approved"] === false]);
  checks.push(["audit_condition.current_formal_run_executed = false", auditCondition!["current_formal_run_executed"] === false]);
  checks.push(["audit_condition.current_formal_execution_status = approval_requested", auditCondition!["current_formal_execution_status"] === "approval_requested"]);
  ...
  if (Array.isArray(priorPhaseChain)) {
    checks.push(["audit_condition prior_phase_chain includes 6.0 through 6.18", priorPhaseChain.length === 19 && priorPhaseChain[0] === "6.0" && priorPhaseChain[18] === "6.18"]);
  }
  ...
  if (Array.isArray(phaseProgressionHistory)) {
    checks.push(["phase_progression_history has 20 entries", phaseProgressionHistory.length === 20]);
    const lastEntry = phaseProgressionHistory[phaseProgressionHistory.length - 1];
    checks.push(["last phase progression = 6.19 dry_run_review_and_formal_run_approval_request_created", lastEntry?.["phase"] === "6.19" && lastEntry?.["outcome"] === "dry_run_review_and_formal_run_approval_request_created"]);
  }
  ...
  if (validationExpectations) {
    ...
    checks.push(["validation_expectations.formal_run_approval_status_must_be = pending_human_review", validationExpectations["formal_run_approval_status_must_be"] === "pending_human_review"]);
    checks.push(["validation_expectations.requested_decision_must_be = approve_formal_offline_batch_run", validationExpectations["requested_decision_must_be"] === "approve_formal_offline_batch_run"]);
    checks.push(["validation_expectations.formal_run_approved_must_be = false", validationExpectations["formal_run_approved_must_be"] === false]);
    ...
    checks.push(["validation_expectations.formal_execution_status_must_be = approval_requested", validationExpectations["formal_execution_status_must_be"] === "approval_requested"]);
  ...
  const forbiddenArtifacts = manifest["forbidden_phase6_19_artifacts"] as Record<string, unknown> | undefined;
  ...
  for (const [label, result] of checks) {
    if (result) {
      console.log(`[validate:dry-run-review-and-formal-run-approval-request] PASS: ${label}`);
    } else {
      console.error(`[validate:dry-run-review-and-formal-run-approval-request] FAIL: ${label}`);
      failed = true;
    }
  }

  console.log(`[validate:dry-run-review-and-formal-run-approval-request] All ${checks.length} invariant checks passed`);
  if (failed) fail("One or more invariant checks failed");
  console.log("[validate:dry-run-review-and-formal-run-approval-request] PASS");
}
